...
export function lintHyperframeHtml(
  ...
  // If a script animates opacity to 0 on dynamically created caption groups
  // (tl.to with opacity: 0) but never uses tl.set with visibility: "hidden",
  // the exit animation can fail due to tween conflicts, leaving captions stuck
  // on screen. This is the #1 caption bug.
  for (const script of scripts) {
    const content = script.content;
    // Detect caption-like pattern: a loop creating elements + exit tween
  ...
  // Rule: caption_container_missing_overflow
  // Caption containers with nowrap text and no max-width will clip off-screen.
  for (const style of styles) {
    const content = style.content;
    // Find selectors with caption-like names
    ...
    for (const [, selector, body] of captionBlocks) {
      ...
      } else if (hasNowrap && !hasOverflow) {
        pushFinding({
          code: "caption_text_overflow_risk",
          severity: "info",
          selector: (selector ?? "").trim(),
          message: `Caption selector "${(selector ?? "").trim()}" has white-space: nowrap without overflow: hidden as a safety net.`,
          fixHint: "Add overflow: hidden to prevent edge-case text overflow.",
        });
      }
    }
  }
