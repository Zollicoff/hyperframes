/**
 * Browser integration test for fitCaptionFontSize.
 *
 * Launches headless Chrome, loads the runtime IIFE into a page,
 * and verifies that window.__hyperframes.fitCaptionFontSize produces
 * correct results with real canvas measureText.
 *
 * Requires: puppeteer (dep of @hyperframes/engine)
 * Run: cd packages/engine && npx tsx scripts/test-fitCaptionFontSize-browser.ts
 */
...
async function main() {
  ...
  try {
    ...
    console.log(
      JSON.stringify({
        event: "fitCaptionFontSize_browser_test_skipped",
...
<script>
  ...
  var r3 = window.__hyperframes.fitCaptionFontSize(
  ...
  window.__testResults.exists = typeof window.__hyperframes.fitCaptionFontSize === "function";
</script>
</body></html>`;
  ...
  try {
    ...
    assert(results.exists === true, "fitCaptionFontSize should exist on window.__hyperframes");
    assert(
    ...
    console.log(
      JSON.stringify({
        event: "fitCaptionFontSize_browser_test_passed",
