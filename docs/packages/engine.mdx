...
The engine includes FFmpeg encoding utilities:
...
await encodeFramesFromDir({ framesDir, outputPath, fps: 30, quality: 'standard' });

// Mix video with audio tracks
await muxVideoWithAudio({ videoPath, audioTracks, outputPath });

// Apply MP4 faststart for streaming
