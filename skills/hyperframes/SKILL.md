...
- Instructions: "Write your output to `.hyperframes/scenes/sceneN.html` as a single file with three clearly marked sections: `<!-- HTML -->`, `<!-- CSS -->`, `<!-- GSAP -->`. Use the timeline variable `tl`. Start tweens at `{start_time}`. Do not create the timeline or register it — the parent handles that. **Use `immediateRender: false` on every `tl.from()` and `tl.fromTo()` call** — without it, the FROM state renders at time 0, making elements invisible before the scene's transition reveals them. **Do NOT set `position`, `top`, `left`, `width`, `height`, `opacity`, or `z-index` on the `#sceneN` container in your CSS** — the scaffold owns those properties. Only style elements INSIDE the scene."
